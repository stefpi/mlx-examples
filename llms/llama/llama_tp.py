...
class Attention(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        # Number of heads per rank (sharded)
        self.n_heads_per_rank = self.n_heads // world_size
        self.n_kv_heads_per_rank = self.n_kv_heads // world_size

        self.repeats = self.n_heads_per_rank // self.n_kv_heads_per_rank

        self.scale = self.args.head_dim**-0.5

        self.wq = nn.QuantizedAllToShardedLinear(args.dim, args.n_heads * args.head_dim, bias=False, group=world)
        self.wk = nn.QuantizedAllToShardedLinear(args.dim, args.n_kv_heads * args.head_dim, bias=False, group=world)
        self.wv = nn.QuantizedAllToShardedLinear(args.dim, args.n_kv_heads * args.head_dim, bias=False, group=world)
        self.wo = nn.QuantizedShardedToAllLinear(args.n_heads * args.head_dim, args.dim, bias=False, group=world)
        self.rope = nn.RoPE(
    ...
    def __call__(
        ...
        queries = queries.reshape(B, L, self.n_heads_per_rank, -1).transpose(0, 2, 1, 3)
        keys = keys.reshape(B, L, self.n_kv_heads_per_rank, -1).transpose(0, 2, 1, 3)
        values = values.reshape(B, L, self.n_kv_heads_per_rank, -1).transpose(0, 2, 1, 3)

        def repeat(a):
            a = mx.concatenate([mx.expand_dims(a, 2)] * self.repeats, axis=2)
            return a.reshape([B, self.n_heads_per_rank, L, -1])
...
# overwrite print to make only rank 0 output to terminal
